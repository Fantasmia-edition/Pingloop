...
interface Props {
  ...
  sellerProfile: { stripe_onboarded: boolean; paypal_onboarded: boolean } | null;
  currentUserId: string | null;
}

function defaultMethod(relay: boolean, home: boolean, pickup: boolean): ShippingMethod | null {
...
export default function BuyerActions({
  ...
  sellerProfile, currentUserId,
}: Props) {
  const hasShipping = shippingRelay || shippingHome || pickupAvailable;
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      {/* Shipping selector — shown before payment */}
      {hasShipping && options.length > 0 && (
        <div className="border border-gray-200 dark:border-navy-700 rounded-xl overflow-hidden">
          <div className="px-4 py-2.5 bg-navy-50 dark:bg-navy-800 border-b border-gray-200 dark:border-navy-700">
            <p className="text-xs font-bold text-navy dark:text-navy-100 uppercase tracking-wide">Mode d&apos;envoi</p>
          </div>
            ...
            {options.map((o) => (
              ...
              >
                ...
                <div className="text-right shrink-0">
                  <p className={`text-sm font-black ${method === o.key ? "text-navy dark:text-lime" : "text-gray-700 dark:text-white"}`}>
