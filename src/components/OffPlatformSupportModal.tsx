...
export default function OffPlatformSupportModal({ listingId, amount, onClose, onSuccess }: Props) {
  const [clientSecret, setClientSecret] = useState<string | null>(null);
  const [error, setError] = useState("");

  useEffect(() => {
    let cancelled = false;
    fetch("/api/stripe/off-platform-support-intent", {
      method: "POST",
    ...
    <div className="fixed inset-0 bg-black/50 flex items-end sm:items-center justify-center z-50 p-4" onClick={onClose}>
      ...
      >
        <div className="flex items-center justify-between">
          <div>
            <h2 className="font-black text-gray-900 dark:text-white">Soutien à PingLoop — {amount.toString().replace(".", ",")} €</h2>
